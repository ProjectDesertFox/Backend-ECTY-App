...
class itineraryPlacesController {
  ...
  static async updateItineraryPlace(req, res, next) {
    try {
      ...
      const itineraryPlace = await ItineraryPlace.update({ name, description, estimatedPrice, rating, itineraryOrder, date, itineraryId, imagePlace }, { where: { id: +req.params.id }, returning: true})
