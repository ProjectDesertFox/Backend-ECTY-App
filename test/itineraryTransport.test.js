    ...
    it('Itinerary create success', function (done) {
        ...
        request(app)
            ...
            .then(response => {
                ...
                expect(body).toHaveProperty("message", "Succes create ItineraryTransportation")
                done()
            })
            .catch(err => {
               // console.log(err,'err============');
...
describe('GET /itineraryTransportation/:id', function () {
    it("Success get one data itineraryTransportation", function (done) {
        request(app)
            .get(`/itineraryTransportation/${idItinerary}`)
            .set("Accept", "application/json")
            .set({ access_token: access_token })
            .then((res) => {
...
describe('PUT /itineraryTransportation/:id', function () {
    it('Update itineraryTransportation', function (done) {
        let input = {
            title : "Bali Trip Abal2",
        }
        request(app)
            .put(`/itineraryTransportation/${idItinerary}`)
            .send(input)
            .set('Accept', 'application/json')
            .set({ access_token: access_token })
            .then(response => {
                ...
                const { body, status } = response
                expect(status).toBe(201)
                expect(body).toHaveProperty("message", `itineraryTransportation with id ${idItinerary} Updated`)
                done()
            })
            .catch(err => {
                console.log(err,'err============');
...
describe('DELETE /itineraryTransportation/:id', function () {
    it("Delete Itinerary", function (done) {
        request(app)
            .delete(`/itineraryTransportation/${idItinerary}`)
            .set("Accept", "application/json")
            .set({ access_token: access_token })
            .then((res) => {
                ...
                const { status, body } = res;
                expect(status).toBe(200);
                expect(body).toHaveProperty("message", `itineraryTransportation with id ${idItinerary} Deleted`)
                //expect body. name ===
